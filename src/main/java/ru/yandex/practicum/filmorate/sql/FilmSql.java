...
public enum FilmSql {
    ...
    INCREMENT_RATE_SQL("UPDATE FILM_DB SET RATE = RATE + 1 WHERE ID = ?"),
    DECREMENT_RATE_SQL(
            "MERGE INTO FILM_DB F USING (SELECT FILM_ID, USER_ID FROM LIKES) L " +
                    "ON (L.FILM_ID = ? AND L.USER_ID = ? AND F.ID = L.FILM_ID) " +
                    "WHEN MATCHED AND F.RATE > 0 THEN UPDATE SET F.RATE = F.RATE - 1"),
